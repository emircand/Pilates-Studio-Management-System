...
/**
 * Performance test for the QRCode entity.
 */
public class QRCodeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all qRCodes").get("/api/qr-codes").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new qRCode")
                        .post("/api/qr-codes")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_qRCode_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created qRCode").get("${new_qRCode_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created qRCode").delete("${new_qRCode_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the QRCode entity").exec(scn);
