...
/**
 * Performance test for the SessionPackage entity.
 */
public class SessionPackageGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all sessionPackages").get("/api/session-packages").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new sessionPackage")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_sessionPackage_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created sessionPackage").get("${new_sessionPackage_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created sessionPackage").delete("${new_sessionPackage_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SessionPackage entity").exec(scn);
